...
export const OpacityHandler = ({id, name, value, setValue}) => {
    ...
    const start = useRef()
    const containerRef = useRef<React.ReactNode>()
    ...
    function handleMove(e) {
        ...
        const position = e.clientX - startX + 15
        const step = 16
    ...
    return (
        <div className={css.opacityContainer} ref={containerRef}>
                ...
                {divs.map((el, id) => {
                    return <span
                        ...
                        style={{
                            ...style,
                            left: `${id * 10}px`,
                            background: `${value * 10 > id ? 'var(--text1)' : 'grey'}`
                        }}
                        onClick={() => {
                            setValue(Math.round((id / 10 + 0.1) * 10) / 10)
                        }}
                    />
                })
                }
                {divs.map((el, id) => {
                    ...
                    return <span
                        ...
                        style={{
                            ...styleCircle,
                            left: `${(id + 1) * 10}px`,
                            borderRight: `1px solid ${value * 10 > id ? 'var(--text1)' : 'grey'}`
                        }}
